...
async function handlePush(data) {
    ...
    if (!(await fs.exists(`repos/${repoUniqueHuman}/Dockerfile`))) throw new Error(`Dockerfile missing for ${data.repository.full_name}!`);
    if (!(await fs.exists(`repos/${repoUniqueHuman}/sdab.json`))) throw new Error(`sdab.json missing for ${data.repository.full_name}!`);
    ...
    if (typeof cfg.tag !== "string") throw new Error(`${data.repository.full_name} sdab.json: tag is not a string or is missing`);
